...
if int(rr.logfile['freq_dump_single_trace']) == 1:
    if id_i in [
        int(rr.logfile['trace_id1']),
        int(rr.logfile['trace_id2']),
        int(rr.logfile['trace_id3']),
    ]:
        use_trace_or_all = "trace"
        
# check if freq_dump_single_all is 1
if int(rr.logfile['freq_dump_single_all']) == 1:
